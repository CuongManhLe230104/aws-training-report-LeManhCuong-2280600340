...
7. **VPC endpoints** (if any) — delete.
8. **Elastic IP** — release the NAT's EIP.
9. **VPC** — delete (removes subnets, route tables, IGW, security groups).

Then remove the standalone services:
10. **SNS** topic + **CloudWatch** alarms + log groups.
11. **ECR** repositories (delete images first).
12. **Cognito** user pool, **SSM** parameters, **ACM** cert (if a custom domain was used).
